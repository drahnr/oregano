...
#define LOG_VIEW_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE((object), TYPE_LOG_VIEW, LogViewPrivate))
...
static void
log_view_finalize (GObject *object)
{
	G_OBJECT_CLASS (log_view_parent_class)->finalize (object);
}

static void
log_view_class_init (LogViewClass *klass)
{
...
static void
log_view_init (LogView *self)
{
	self->priv = LOG_VIEW_GET_PRIVATE (self);
}
...
log_view_new ()
{
	...
	col1 = gtk_tree_view_column_new_with_attributes ("Source",
	                                                  renderer,
	                                                  "text", 0,
	                                                  NULL);

	col2 = gtk_tree_view_column_new_with_attributes ("Message",
	                                                 renderer,
	                                                 "text", 1,
	                                                 NULL);
...
void
log_view_set_store (LogView *lv, Log *ls)
{
